...
def main(pm=pm):
    ...
    for itteration in range(cdt.max_iter):
        ...
        for j, ray in enumerate(tqdm(cdt.rays, desc=f'propagating rays itteration {itteration}', ncols=90)):
    ...
    if cdt.extra_save:
        ...
        for KK in range(3):
            for QQ in range(KK+1):
                ...
                if st.atomic[0].atom.lines[0].separate_components:
                ...
                for lay, heigt in enumerate(st.atomic):
                    if heigt.atom.lines[0].separate_components:
